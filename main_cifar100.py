...
def shuffle_minibatch(inputs, targets, mixup=True):
    ...
    Args:
        inputs: a numpy array of images with size batch_size x 3 x H x W.
    ...
    #random index inpus1 & inputs2 / targets1_1 & targets2_1

    y_onehot = torch.FloatTensor(batch_size, num_classes)
    y_onehot.zero_() # fill tensor with zeros / torch.Size([128,100])
    targets1_oh = y_onehot.scatter_(1, targets1_1, 1) # .scatter_(dim, index, src) write all values from the tensor src into y_onehot at indices specified in the index tensor / torch.Size([128,100])
    ...
    if mixup is True:
        a = numpy.random.beta(1, 1, [batch_size, 1]) # beta distribution # f(x;a,b) / random.beta(a,b,size=None) / a.shape = (128,1) / class 'numpy.ndarray'
    else:
        a = numpy.ones((batch_size, 1))
                                                       # a[..., None, None] = (128,1) -> (128,1,1,1) to 4 dim // expands dimension with the # of None
    b = numpy.tile(a[..., None, None], [1, 3, 32, 32]) # numpy.tile(A,reps) Construct an array by repeating A the number of times given by reps.

    inputs1 = inputs1 * torch.from_numpy(b).float() # creates a tensor from a numpy.ndarray / torch.from_numpy(ndarray) → Tensor / multiply weights
